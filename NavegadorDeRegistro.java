import java.sql.*;
import java.util.*; // importa bibliotecas relevantes para a aplicação

public class NavegadorDeRegistro extends TelaDeAtualizacao { // cria classe que herda TelaDeAtualização
    public static void popularIds() { 
        try {
            ArrayList<String> idsTemp = new ArrayList<>(); // cria array list
            idsTemp.add("Selecione aqui o id"); // define uma String para ser exibida ao usuario
            Connection conexao = MySQLConnector.conectar(); // executa o metodo conectar
            ...
            ResultSet rstSqlPopularIds = stmSqlPopularIds.executeQuery(strSqlPopularIds); // define e executa query no sql para obter ids da tbl_senac em ordem ascendente
            while (rstSqlPopularIds.next()) { // cria condicional: enquanto houver um proximo id 
                idsTemp.add(rstSqlPopularIds.getString("id")); // adciona o id em questao
            }
            ids = idsTemp.toArray(new String[0]); // faz conversao de tipos 
            ...
            System.err.println("Erro: " + e); // define texto para ser notificado em caso de exceção e imprime uma mensagem mais tecnica para o dev
        }
    }

    public static void atualizarId() { // cria o metodo atualizarId, utilizado em TelaDeAtualização
        try {
            String atualizarNome = "";
            String atualizarEmail = ""; // cria Strings vazias para "limpar" inputs quando necessário
            String atualizarSenha = "";

            if (txtNome.getText().trim().equals(nomeAtual) == false) { // se o nome atual for diferente do conteudo do input
                atualizarNome = "`nome` = '" + txtNome.getText() + "'"; // cria uma query SQL
            }

            if (txtEmail.getText().trim().equals(emailAtual) == false) { // a mesma coisa só que para email
                if (atualizarNome.length() > 0) { // se a quantidade de caracteres de atualizarNome for maior que 0
                    atualizarEmail = " and ";   // cria query   
                }
                atualizarEmail += "`email` = '" + txtEmail.getText() + "'"; // cria query SQL
            }

            if (String.valueOf(txtSenha.getPassword()).trim().equals(senhaAtual) == false) { // se o conteudo do input for diferente da senhaAtual 
                if (atualizarNome.length() > 0 || atualizarEmail.length() > 0) { // verifica se atualizarNome é maior q 0 ou se atualizarEmail é maior que 0
                    atualizarSenha = " and "; // se atendida a condicional, cria mais uma query SQL
                }
                atualizarSenha += "`senha` = '" + String.valueOf(txtSenha.getPassword()) + "'"; // cria mais uam querySQL
            }
